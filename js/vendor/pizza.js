    ...
    $('li', legend).each(function () {
      ...
      if (options.data) {
        ...
        data.push({
          ...
          text: segment.data('text'),
          //faizan edit 
          color: segment.data('color') != '' && segment.data('color') != null ? segment.data('color') : segment.css('color'),
          segment: segment
        });
      }
    });
...
$.extend(Pizza, {
  line : function (legend) {
    ...
    this.set_attr(polyline, {d:v, fill: 'none', stroke: 'black', 'stroke-width': settings.stroke_width});
